---
name: everything-claude-code__autonomous-loops
description: 自主循环技能
version: 1.0.0
...
在循环中自主运行 Your coding agent 的模式、架构和参考实现。涵盖从简单的 `claude -p` 管道到完整的 RFC 驱动的多智能体 DAG 编排的一切。
...
`claude -p` 标志以非交互方式运行 Your coding agent 并附带提示，完成后退出。链式调用来构建管道：
...
1. 从 `~/.agent/claw/{session}.md` 加载对话历史
...
### 通过 Your coding agent 命令实现

创建 `.agent/commands/infinite.md`：
...
**一个生产级的 shell 脚本**，在持续循环中运行 Your coding agent，创建 PR，等待 CI，并自动合并。由 AnandChowdhary 创建（致谢：@AnandChowdhary）。
