...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.agent/skills/gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Your coding agent) can call it as both a CLI and an MCP tool.

**Scope honesty:** This skill's MCP registration step (5a) uses
`claude mcp add` and targets Your coding agent specifically. Other local hosts
...
local Your coding agent is only possible through shared Postgres (Supabase).
...
```bash
~/.agent/skills/gstack/bin/gstack-gbrain-detect
```
...
- **1 — Supabase, I already have a connection string.** Cloud-agent users
  ...
  URL gives your local Your coding agent full read/write access to every page your
...
```bash
~/.agent/skills/gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.agent/skills/gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.agent/skills/gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.agent/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.agent/skills/gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.agent/skills/gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.agent/skills/gstack/bin/gstack-gbrain-supabase-provision \
...
## Step 5a: Register gbrain as Your coding agent MCP (D18)

Only if `which claude` resolves. Ask: "Give Your coding agent a typed tool surface
for gbrain? (recommended yes)"

If yes, register at **user scope** with an **absolute path** to the gbrain
binary. User scope makes the MCP available in every Your coding agent session on
this machine, not just the current workspace. Absolute path avoids PATH
resolution issues when Your coding agent spawns `gbrain serve` as a subprocess.
...
**Heads-up for the user:** an already-open Your coding agent session will not
pick up the new MCP tools until restart. Tell them: "Restart any open
Your coding agent sessions to see `mcp__gbrain__*` tools — they're loaded at
...
```bash
current_tier=$(~/.agent/skills/gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.agent/skills/gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
```bash
~/.agent/skills/gstack/bin/gstack-brain-init
~/.agent/skills/gstack/bin/gstack-config set gbrain_sync_mode artifacts-only
# or "full" if user picked yes-full
```

Then wire the brain repo into gbrain so its content is searchable from any
gbrain client (this Your coding agent session, future Macs, optional cloud agents).
...
try:
    ...
    pass
")
~/.agent/skills/gstack/bin/gstack-gbrain-source-wireup --strict \
...
## Step 8: Persist `## GBrain Configuration` in SKILL.md

Find-and-replace (or append) this section in SKILL.md:
...
- **SKILL.md is the audit trail.** Always update it in Step 8 after a
  successful setup.
