...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.agent/skills/gstack/bin/gstack-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.agent/skills/gstack/browse/dist/browse" ] && B="$_ROOT/.agent/skills/gstack/browse/dist/browse"
[ -z "$B" ] && B="$HOME/.agent/skills/gstack/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' SKILL.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
...
```bash
# Check for persisted deploy config in SKILL.md
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" SKILL.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in SKILL.md, use them directly
...
and any persisted config from SKILL.md.
...
1. **SKILL.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" SKILL.md 2>/dev/null | head -3
```
...
```bash
~/.agent/skills/gstack/bin/gstack-review-read 2>/dev/null
```
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to SKILL.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
```bash
cat ~/.agent/skills/gstack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
Read SKILL.md to find the project's test command. If not specified, use `bun test`.
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md SKILL.md VERSION
```
...
```bash
eval $(~/.agent/skills/gstack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
If staging was detected in Step 1.5c (or from SKILL.md deploy config), and the changes
...
If a deploy status command was configured in SKILL.md (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If SKILL.md has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
