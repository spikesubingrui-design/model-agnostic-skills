---
name: hermes-agent__claude-code
description: Your coding agent — Hermes Orchestration Guide
version: 1.0.0
...
---
name: claude-code
description: "Delegate coding to Your coding agent CLI (features, PRs)."
version: 2.2.0
...
# Your coding agent — Hermes Orchestration Guide

Delegate coding tasks to [Your coding agent](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Hermes terminal. Your coding agent v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Hermes interacts with Your coding agent in two fundamentally different ways. Choose based on the task.
...
# Launch Your coding agent inside it
...
Your coding agent presents up to two confirmation dialogs on first launch. You MUST handle these via tmux send-keys:
...
| `claude update` / `claude upgrade` | Update Your coding agent to latest version |
...
| `claude plugin` / `claude plugins` | Manage Your coding agent plugins |
...
`--bare` skips hooks, plugins, MCP discovery, and SKILL.md loading. Fastest startup. Requires `ANTHROPIC_API_KEY` (skips OAuth).
...
| `-w, --worktree [name]` | Run in an isolated git worktree at `.agent/worktrees/<name>` |
...
| `--bare` | Skip hooks, plugins, MCP discovery, SKILL.md, OAuth (fastest startup) |
...
2. **Local project:** `.agent/settings.local.json` (personal, gitignored)
3. **Project:** `.agent/settings.json` (shared, git-tracked)
4. **User:** `~/.agent/settings.json` (global)
...
### Memory Files (SKILL.md) Hierarchy
1. **Global:** `~/.agent/SKILL.md` — applies to all projects
2. **Project:** `./SKILL.md` — project-specific context (git-tracked)
3. **Local:** `.agent/CLAUDE.local.md` — personal project overrides (gitignored)
...
| `/compact [focus]` | Compress context to save tokens; SKILL.md survives compaction. E.g., `/compact focus on auth logic` |
...
| `/init` | Create a SKILL.md file for project memory |
| `/memory` | Open SKILL.md for editing |
...
Create `.agent/commands/<name>.md` (project-shared) or `~/.agent/commands/<name>.md` (personal):
...
Unlike slash commands (manually invoked), skills in `.agent/skills/` are markdown guides that Claude invokes automatically via natural language when the task matches:

```markdown
# .agent/skills/database-migration.md
...
| `#` | Quick add to SKILL.md memory (e.g., `# Use 2-space indentation`) |
| `/` | Slash commands |
...
```
Creates an isolated git worktree at `.agent/worktrees/feature-x` AND a tmux session for it. Uses iTerm2 native panes when available; add `--tmux=classic` for traditional tmux.
...
Your coding agent auto-loads `SKILL.md` from the project root. Use it to persist project context:
...
### Rules Directory (Modular SKILL.md)
For projects with many rules, use the rules directory instead of one massive SKILL.md:
- **Project rules:** `.agent/rules/*.md` — team-shared, git-tracked
- **User rules:** `~/.agent/rules/*.md` — personal, global

Each `.md` file in the rules directory is loaded as additional context. This is cleaner than cramming everything into a single SKILL.md.

### Auto-Memory
Claude automatically stores learned project context in `~/.agent/projects/<project>/memory/`.
- **Limit:** 25KB or 200 lines per project
- This is separate from SKILL.md — it's Claude's own notes about the project, accumulated across sessions

## Custom Subagents

Define specialized agents in `.agent/agents/` (project), `~/.agent/agents/` (personal), or via `--agents` CLI flag (session):

### Agent Location Priority
1. `.agent/agents/` — project-level, team-shared
2. `--agents` CLI flag — session-specific, dynamic
3. `~/.agent/agents/` — user-level, personal
...
Configure in `.agent/settings.json` (project) or `~/.agent/settings.json` (global):
...
| `-s local` | This project (personal) | `.agent/settings.local.json` (gitignored) |
| `-s project` | This project (team-shared) | `.agent/settings.json` (git-tracked) |
...
1. **Interactive mode REQUIRES tmux** — Your coding agent is a full TUI app. Using `pty=true` alone in Hermes terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
