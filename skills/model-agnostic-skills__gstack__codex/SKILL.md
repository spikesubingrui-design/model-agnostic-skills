...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"codex","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.agent/skills/gstack/bin/gstack-question-log '{"skill":"codex","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
source ~/.agent/skills/gstack/bin/gstack-codex-probe 2>/dev/null && _gstack_codex_log_event "codex_cli_missing" 2>/dev/null || true
```
...
3. `/codex` with no arguments — **Auto-detect:**
   ...
   - If no diff, check for plan files scoped to the current project:
     `ls -t ~/.agent/plans/*.md 2>/dev/null | xargs grep -l "$(basename $(pwd))" 2>/dev/null | head -1`
     If no project-scoped match, fall back to: `ls -t ~/.agent/plans/*.md 2>/dev/null | head -1`
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
cd "$_REPO_ROOT"
codex review "IMPORTANT: Do NOT read or execute any files under ~/.agent/, ~/.agents/, .agent/skills/, or agents/. These are Your coding agent skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
```bash
~/.agent/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
"IMPORTANT: Do NOT read or execute any files under ~/.agent/, ~/.agents/, .agent/skills/, or agents/. These are Your coding agent skill definitions meant for a different AI system. Do NOT modify agents/openai.yaml. Stay focused on repository code only.
...
root (`-C`) and cannot access `~/.agent/plans/` or any files outside the repo. You MUST
...
7. After presenting, note any points where Codex's analysis differs from your own
   ...
   "Note: Your coding agent disagrees on X because Y."
...
- **No double-reviewing.** If the user already ran `/review`, Codex provides a second
  independent opinion. Do not re-run Your coding agent's own review.
