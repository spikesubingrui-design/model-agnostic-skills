---
name: claude-mem__openclaw
description: Claude-Mem OpenClaw Plugin — Setup Guide
version: 1.0.0
...
If this machine also runs Your coding agent with claude-mem installed, the worker may already be running:
...
#### If Your coding agent has claude-mem installed

If claude-mem is installed as a Your coding agent plugin (at `~/.agent/plugins/marketplaces/thedotmack/`), start the worker from that installation:

```bash
cd ~/.agent/plugins/marketplaces/thedotmack
...
#### If there's no Your coding agent installation
...
| Worker started from Your coding agent install but not responding | Check `cd ~/.agent/plugins/marketplaces/thedotmack && npm run worker:status`. May need `npm run worker:restart`. |
