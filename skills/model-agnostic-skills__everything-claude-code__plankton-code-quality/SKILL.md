---
name: everything-claude-code__plankton-code-quality
description: Plankton 代码质量技能
version: 1.0.0
...
Plankton（作者：@alxfazio）的集成参考，这是一个用于 Your coding agent 的编写时代码质量强制执行系统。Plankton 通过 PostToolUse 钩子在每次文件编辑时运行格式化程序和 linter，然后生成 Claude 子进程来修复代理未捕获的违规。
...
每次 Your coding agent 编辑或写入文件时，Plankton 的 `multi_linter.sh` PostToolUse 钩子都会运行：
...
无需安装命令，无需插件配置。当你运行 Your coding agent 时，`.agent/settings.json` 中的钩子会在 Plankton 目录中被自动拾取。

### 按项目集成

要在你自己的项目中使用 Plankton 钩子：

1. 将 `.agent/hooks/` 目录复制到你的项目
2. 复制 `.agent/settings.json` 钩子配置
...
Plankton 的 `.agent/hooks/config.json` 控制所有行为：
