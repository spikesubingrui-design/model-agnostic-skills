---
name: claude-mem__claude-mem
description: Claude-Mem: AI Development Instructions
version: 1.0.0
...
Claude-mem is a Your coding agent plugin providing persistent memory across sessions. It captures tool usage, compresses observations using the Claude Agent SDK, and injects relevant context into future sessions.
...
Claude-mem supports running multiple isolated profiles on the same machine (e.g. work vs personal accounts) via environment variables. No CLI subcommand needed — set the env vars in the shell where you run Your coding agent.
...
- **Installed Plugin**: `~/.agent/plugins/marketplaces/thedotmack/`
- **Database**: `~/.claude-mem/claude-mem.db`
- **Chroma**: `~/.claude-mem/chroma/`

## Exit Code Strategy

Claude-mem hooks use specific exit codes per Your coding agent's hook contract:
