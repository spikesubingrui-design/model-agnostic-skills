---
name: everything-claude-code__team-builder
description: 团队构建器
version: 1.0.0
...
2. `~/.agent/agents/**/*.md` + `~/.agent/agents/*.md` — 全局智能体（两种深度）
...
* **并行 Agent 调用，而非 TeamCreate。** 此技能使用并行 Agent 工具调用来处理独立工作。只有当智能体需要辩论或相互回应时，才需要 TeamCreate（一个用于多智能体对话的 Your coding agent 工具）。
