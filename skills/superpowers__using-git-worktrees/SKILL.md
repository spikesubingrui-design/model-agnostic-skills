...
```bash
grep -i "worktree.*director" SKILL.md 2>/dev/null
```
...
If no directory exists and no SKILL.md preference:
...
| Neither exists | Check SKILL.md → Ask user |
...
- **Fix:** Follow priority: existing > SKILL.md > ask
...
**Always:**
- Follow directory priority: existing > SKILL.md > ask
