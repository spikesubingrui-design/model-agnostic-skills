...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   See `.agent/docs/director-gates.md` for the full check pattern.
...
- **`creative-director`** — gate **CD-PILLARS** (`.agent/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`.agent/docs/director-gates.md`)
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`.agent/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`.agent/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `.agent/docs/templates/game-concept.md`. Fill in ALL sections from the
...
If yes, generate the document using the template at `.agent/docs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
...
> to `design/gdd/game-concept.md`. Open a fresh Your coding agent session to continue
