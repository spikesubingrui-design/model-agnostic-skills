...
**Required Artifacts:**
- [ ] Engine chosen (SKILL.md Technology Stack is not `[CHOOSE]`)
- [ ] Technical preferences configured (`.agent/docs/technical-preferences.md` populated)
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via Task using the parallel gate protocol from `.agent/docs/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.

**Spawn in parallel:**

1. **`creative-director`** — gate **CD-PHASE-GATE** (`.agent/docs/director-gates.md`)
2. **`technical-director`** — gate **TD-PHASE-GATE** (`.agent/docs/director-gates.md`)
3. **`producer`** — gate **PR-PHASE-GATE** (`.agent/docs/director-gates.md`)
4. **`art-director`** — gate **AD-PHASE-GATE** (`.agent/docs/director-gates.md`)
...
- **No accessibility requirements doc?** → Use `AskUserQuestion` to offer to create it now:
  ...
  - Then write `design/accessibility-requirements.md` using the template at `.agent/docs/templates/accessibility-requirements.md`, filling in the chosen tier. Confirm: "May I write `design/accessibility-requirements.md`?"
