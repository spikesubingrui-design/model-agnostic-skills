---
name: hermes-agent__gitnexus-explorer
description: GitNexus Explorer
version: 1.0.0
...
rm -rf .agent/    # remove Your coding agent-specific artifacts
```
...
- **The web UI does NOT create `.agent/` or `SKILL.md`.** Those are created by
  `npx gitnexus analyze`. Use `--skip-agents-md` to suppress the markdown files,
  then `rm -rf .agent/` for the rest. These are Your coding agent integrations that
