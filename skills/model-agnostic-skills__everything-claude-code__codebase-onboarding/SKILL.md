---
name: everything-claude-code__codebase-onboarding
description: 代码库入门引导
version: 1.0.0
...
系统性地分析一个不熟悉的代码库，并生成结构化的入门指南。专为加入新项目的开发者或首次在现有仓库中设置 Your coding agent 的用户设计。

## 使用时机

* 首次使用 Your coding agent 打开项目时
* 加入新团队或新仓库时
* 用户询问“帮我理解这个代码库”
* 用户要求为项目生成 SKILL.md 文件
* 用户说“带我入门”或“带我浏览这个仓库”
...
根据检测到的规范，生成或更新项目特定的 SKILL.md。如果 `SKILL.md` 已存在，请先读取它并进行增强——保留现有的项目特定说明，并明确标注新增或更改的内容。
...
**用户**：“带我入门这个代码库”
**操作**：运行完整的 4 阶段工作流 → 生成入门指南 + 初始 SKILL.md
**输出**：入门指南直接打印到对话中，并在项目根目录写入一个 `SKILL.md`

### 示例 2：为现有项目生成 SKILL.md

**用户**：“为这个项目生成一个 SKILL.md”
**操作**：运行阶段 1-3，跳过入门指南，仅生成 SKILL.md
**输出**：包含检测到的规范的项目特定 `SKILL.md`

### 示例 3：增强现有的 SKILL.md

**用户**：“用当前项目规范更新 SKILL.md”
**操作**：读取现有 SKILL.md，运行阶段 1-3，合并新发现
**输出**：更新后的 `SKILL.md`，并明确标记了新增内容
