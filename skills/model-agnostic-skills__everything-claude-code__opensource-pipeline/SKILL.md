---
name: everything-claude-code__opensource-pipeline
description: Open-Source Pipeline Skill
version: 1.0.0
...
compatible_models: [deepseek, gpt-4o, gpt-4.5, claude-3.5-sonnet, claude-4, qwen-max, qwen-plus, gemini-2.5-pro]
compatible_frameworks: [openclaw, cursor, continue-dev, windsurf]
tags: [agent, testing, review, planning]
---
...
Safely open-source any project through a 3-stage pipeline: **Fork** (strip secrets) → **Sanitize** (verify clean) → **Package** (SKILL.md + setup.sh + README).
...
| `/opensource package PROJECT` | Generate SKILL.md + setup.sh + README |
...
Generate:
1. SKILL.md (commands, architecture, key files)
...
Show each project with pipeline progress (FORK_REPORT.md, SANITIZATION_REPORT.md, SKILL.md presence).
