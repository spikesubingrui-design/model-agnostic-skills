---
name: everything-claude-code__continuous-learning-v2
description: Continuous Learning v2 - インスティンクトベースアーキテクチャ
version: 1.0.0
...
`~/.agent/settings.json`に追加します。

**プラグインとしてインストールした場合**（推奨）：

```json
プラグインの `hooks/hooks.json` が Your coding agent v2.1+ で自動読み込みされるため、`~/.agent/settings.json` に追加の hook 設定は不要です。`observe.sh` はそこで既に登録されています。

以前に `observe.sh` を `~/.agent/settings.json` にコピーした場合は、重複した `PreToolUse` / `PostToolUse` ブロックを削除してください。重複登録は二重実行と `${CLAUDE_PLUGIN_ROOT}` 解決エラーを引き起こします。この変数はプラグイン管理の `hooks/hooks.json` でのみ展開されます。

**`~/.agent/skills`に手動でインストールした場合**：
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "~/.agent/skills/continuous-learning-v2/hooks/observe.sh"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
...
v2はv1と完全に互換性があります：
- 既存の`~/.agent/skills/learned/`スキルは引き続き機能
