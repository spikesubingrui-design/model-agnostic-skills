---
name: everything-claude-code__agent-eval
description: Agent Eval 技能
version: 1.0.0
...
---
name: agent-eval
description: 编码代理（Your coding agent、Aider、Codex等）在自定义任务上的直接比较，包含通过率、成本、时间和一致性指标
origin: ECC
...
* 在你自己的代码库上比较编码代理（Your coding agent、Aider、Codex 等）
* 在采用新工具或模型之前衡量代理性能
* 当代理更新其模型或工具时运行回归检查
* 为团队做出数据支持的代理选择决策
