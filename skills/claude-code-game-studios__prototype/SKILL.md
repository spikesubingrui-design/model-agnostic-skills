...
Read `SKILL.md` for project context and the current tech stack. Understand what engine, language, and frameworks are in use so the prototype is built with compatible tooling.
...
Spawn `creative-director` via Task using gate **CD-PLAYTEST** (`.agent/docs/director-gates.md`).
