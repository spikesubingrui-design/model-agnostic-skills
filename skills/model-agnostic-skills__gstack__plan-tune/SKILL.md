...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.agent/skills/gstack/bin/gstack-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
7. **"Turn it off" / "disable"** → `~/.agent/skills/gstack/bin/gstack-config set question_tuning false`
8. **"Turn it on" / "enable"** → `~/.agent/skills/gstack/bin/gstack-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$(~/.agent/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
3. If A or B: enable:
   ```bash
   ~/.agent/skills/gstack/bin/gstack-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.agent/skills/gstack/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.agent/skills/gstack/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.agent/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.agent/skills/gstack/bin/gstack-developer-profile --gap
```
...
```bash
~/.agent/skills/gstack/bin/gstack-question-preference --stats
eval "$(~/.agent/skills/gstack/bin/gstack-slug 2>/dev/null)"
_LOG="${GSTACK_HOME:-$HOME/.gstack}/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
~/.agent/skills/gstack/bin/gstack-developer-profile --profile | bun -e "
