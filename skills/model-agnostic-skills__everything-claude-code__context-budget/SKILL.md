---
name: everything-claude-code__context-budget
description: 上下文预算
version: 1.0.0
...
分析 Your coding agent 会话中每个已加载组件的令牌开销，并提供可操作的优化建议以回收上下文空间。
...
| 类别 | 标准 | 操作 |
|--------|----------|--------|
| **始终需要** | 在 SKILL.md 中被引用，支持活动命令，或匹配当前项目类型 | 保留 |
| **有时需要** | 特定领域（例如语言模式），未在 SKILL.md 中引用 | 考虑按需激活 |
...
* **冗余组件** — 重复代理逻辑的技能，重复 SKILL.md 的规则
* **MCP 超额订阅** — >10 个服务器，或包装了可免费使用的 CLI 工具的服务器
* **SKILL.md 臃肿** — 冗长的解释、过时的部分、本应成为规则的指令
