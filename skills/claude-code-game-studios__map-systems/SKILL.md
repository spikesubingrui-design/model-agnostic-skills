...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`.agent/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`.agent/docs/director-gates.md`) before writing the index.**
...
Using the template at `.agent/docs/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`.agent/docs/director-gates.md`).**
...
> `design/gdd/systems-index.md`. Open a fresh Your coding agent session to continue
