---
name: gstack
description: gstack development
version: 1.0.0
...
1. **Read SKILL.md** for project-specific config (test commands, eval commands, etc.)
2. **If missing, AskUserQuestion** — let the user tell you or let gstack search the repo
3. **Persist the answer to SKILL.md** so we never have to ask again
...
When developing gstack, `.agent/skills/gstack` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Your coding agent sessions using gstack concurrently.

**Check once per session:** Run `ls -la .agent/skills/gstack` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all gstack invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent gstack sessions
- During large refactors, remove the symlink (`rm .agent/skills/gstack`) so the
  global install at `~/.agent/skills/gstack/` is used instead
...
The active skill lives at `~/.agent/skills/gstack/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.agent/skills/gstack && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.agent/skills/gstack && bun run build`

Or copy the binaries directly:
- `cp browse/dist/browse ~/.agent/skills/gstack/browse/dist/browse`
- `cp design/dist/design ~/.agent/skills/gstack/design/dist/design`
