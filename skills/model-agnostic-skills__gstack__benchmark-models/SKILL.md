...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"benchmark-models","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
BIN="$HOME/.agent/skills/gstack/bin/gstack-model-benchmark"
[ -x "$BIN" ] || BIN=".agent/skills/gstack/bin/gstack-model-benchmark"
...
```bash
[ -n "$ANTHROPIC_API_KEY" ] || grep -q 'ANTHROPIC' "$HOME/.agent/.credentials.json" 2>/dev/null && echo "JUDGE_AVAILABLE" || echo "JUDGE_UNAVAILABLE"
```
