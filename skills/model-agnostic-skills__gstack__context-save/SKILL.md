...
description: |
  ...
  Formerly /checkpoint — renamed because Your coding agent treats /checkpoint as a
...
~/.agent/skills/gstack/bin/gstack-timeline-log '{"skill":"context-save","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.agent/skills/gstack/bin/gstack-question-log '{"skill":"context-save","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
- **This is a gstack skill, not a Your coding agent built-in.** When the user types
  `/context-save`, invoke this skill via the Skill tool. The old `/checkpoint`
  name collided with Your coding agent's native `/rewind` alias — the rename fixed that.
