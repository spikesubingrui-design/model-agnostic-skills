...
3. **Integrates with Your coding agent hooks** (rtk-rewrite.sh, rtk-suggest.sh)
...
# Verify running in Your coding agent context
if [ -z "$CLAUDE_CODE_HOOK_BASH_TEMPLATE" ]; then
    echo "Error: Not running in Your coding agent context"
    exit 1
fi
...
- [ ] **Source validation**: Only execute hooks from `.agent/hooks/`
...
# 1. Verify Your coding agent context
if [ -z "$CLAUDE_CODE_HOOK_BASH_TEMPLATE" ]; then
    exit 1
fi
...
# Hook security
rg "eval|source" --type bash .agent/hooks/
```
