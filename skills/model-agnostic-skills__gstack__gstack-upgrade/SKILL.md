...
tags: [coding, agent, testing, review]
---
...
[ -z "$_AUTO" ] && _AUTO=$(~/.agent/skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
~/.agent/skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
~/.agent/skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `~/.agent/skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/.agent/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.agent/skills/gstack"
elif [ -d "$HOME/.gstack/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.gstack/repos/gstack"
elif [ -d ".agent/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agent/skills/gstack"
elif [ -d ".agents/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/gstack"
elif [ -d ".agent/skills/gstack" ]; then
  INSTALL_TYPE="vendored"
  INSTALL_DIR=".agent/skills/gstack"
elif [ -d "$HOME/.agent/skills/gstack" ]; then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.agent/skills/gstack" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.agent/skills/gstack" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.agent/skills/gstack"
  fi
fi
_TEAM_MODE=$(~/.agent/skills/gstack/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached .agent/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.agent/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.agent/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `.agent/skills/gstack/` when you're ready."
...
```bash
~/.agent/skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
.agent/skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.agent/skills/gstack/` when you're ready."
